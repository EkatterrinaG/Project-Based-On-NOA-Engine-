...
//                var val = raster[offset];
    ...
    this.pixelate = function(pixelSize)
    {
        var newData = new Array(this.width * this.height * 4);
        
        for (var j = 0; j < this.height; j += pixelSize)
        {
            for (var i = 0; i < this.width; i += pixelSize)
            {
                // Вычисляем средний цвет для блока пикселей
                var rSum = 0, gSum = 0, bSum = 0, count = 0;
                
                for (var dy = 0; dy < pixelSize && (j + dy) < this.height; dy++)
                {
                    for (var dx = 0; dx < pixelSize && (i + dx) < this.width; dx++)
                    {
                        var offset = ((j + dy) * this.width + (i + dx)) * 4;
                ...
                // Квантуем цвета для более каменного вида (ограниченное количество оттенков)
                var levels = 12; // Количество уровней серого для каменной текстуры
                var avgGray = (avgR + avgG + avgB) / 3;
                var quantized = Math.round(avgGray / (256 / levels)) * (256 / levels);
                quantized = Math.max(0, Math.min(255, quantized));
                
                // Применяем средний цвет ко всему блоку
                for (var dy = 0; dy < pixelSize && (j + dy) < this.height; dy++)
                {
                    for (var dx = 0; dx < pixelSize && (i + dx) < this.width; dx++)
                    {
                        var offset = ((j + dy) * this.width + (i + dx)) * 4;
                        newData[offset] = quantized;
                        newData[offset + 1] = quantized;
                        newData[offset + 2] = quantized;
                        newData[offset + 3] = this.data[offset + 3];
                    }
                }
            }
        }
    ...
    this.addBorder = function(borderWidth, borderColor)
    {
        if (!borderColor)
        {
            borderColor = {r: 50, g: 50, b: 50, a: 255}; // Темно-серый по умолчанию
        }
        
        // Рисуем верхнюю и нижнюю границы
        for (var i = 0; i < this.width; i++)
        {
            for (var bw = 0; bw < borderWidth; bw++)
            {
                // Верхняя граница
                if (bw < this.height)
                {
                    this.setRgba(i, bw, borderColor.r, borderColor.g, borderColor.b, borderColor.a);
                }
                // Нижняя граница
                if (this.height - 1 - bw >= 0)
                {
                    this.setRgba(i, this.height - 1 - bw, borderColor.r, borderColor.g, borderColor.b, borderColor.a);
                }
            }
        }
        ...
        for (var j = 0; j < this.height; j++)
        {
            for (var bw = 0; bw < borderWidth; bw++)
            {
                ...
                if (bw < this.width)
                {
                    this.setRgba(bw, j, borderColor.r, borderColor.g, borderColor.b, borderColor.a);
                }
                // Правая граница
                if (this.width - 1 - bw >= 0)
                {
                    this.setRgba(this.width - 1 - bw, j, borderColor.r, borderColor.g, borderColor.b, borderColor.a);
                }
            }
        }
    };
